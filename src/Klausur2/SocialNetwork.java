...
public class SocialNetwork {
	...
	public ArrayList<User> findAllFriends(User anfang, int level, int maxDepth, ArrayList<User> users){
		for(User u: anfang.getFriends()){
			if(level > maxDepth){
				return users;
			}
			...
			else return findAllFriends(u, level+1, maxDepth, users);
		}
		return users;
